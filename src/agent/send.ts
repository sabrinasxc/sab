...
export async function sendDraftForAgentRun(agentRunId: string, actor: { actorType: "AI" | "USER"; actorId: string | null; approvalSatisfied: boolean; autoEmailAllowed?: boolean }) {
  ...
  const guard = authorizeExecution({ organizationId: business.organization_id, businessId: run.business_id, mailboxId: run.mailbox_id, action: "SEND_EMAIL", risk: run.risk, confidence: Number(run.confidence ?? 0), autonomyLevel: business.autonomy_level, globalAiActive: switches.aiActive, globalAutoSendActive: actor.approvalSatisfied ? true : switches.autoSendActive, businessAiActive: business.ai_active, businessAutoSendActive: actor.approvalSatisfied ? true : business.auto_send_active, mailboxAiActive: mailbox.ai_active, mailboxAutoSendActive: actor.approvalSatisfied ? true : mailbox.auto_send_active, actionAllowedByBusinessRule: actor.approvalSatisfied || Boolean(actor.autoEmailAllowed), recipientAuthorized: true, duplicateDetected: Boolean(alreadySent), prohibitedCategory: run.risk === "CRITICAL" && !actor.approvalSatisfied, attachmentPermissionSatisfied: true, approvalSatisfied: actor.approvalSatisfied });
