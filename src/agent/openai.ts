...
export async function decideEmail(input: { businessName: string; thread: string; triage: TriageResult; crm: unknown; rules: unknown; knowledge: unknown }): Promise<DecisionResult> {
...
export async function draftEmail(input: { businessName: string; thread: string; triage: TriageResult; decision: DecisionResult; crm: unknown; knowledge: unknown }): Promise<ResponseDraft> {
  const raw = await structured<unknown>("email_response", responseJsonSchema, `You are the RESPONSE pass for ${input.businessName}. Write only from verified thread, CRM, and retrieved business knowledge. Do not invent facts, prices, dates, commitments, legal conclusions, refunds, payment changes, or actions not present in context. Use concise natural paragraphs. Do not use em dashes.\n\nTRIAGE:\n${JSON.stringify(input.triage)}\n\nDECISION:\n${JSON.stringify(input.decision)}\n\nCRM:\n${JSON.stringify(input.crm)}\n\nKNOWLEDGE:\n${JSON.stringify(input.knowledge)}\n\nTHREAD:\n${input.thread}`);
