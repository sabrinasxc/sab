...
const triageJsonSchema = {
  ...
  required: ["category", "subcategory", "risk", "sentiment", "confidence", "promptInjectionSuspected", "summary"],
  properties: {
...
const decisionJsonSchema = {
  type: "object",
  additionalProperties: false,
  required: ["action", "confidence", "risk", "reasoningSummary", "requiresResponse", "requiresApproval", "proposedToolActions"],
  properties: {
    action: { type: "string", enum: ["NO_ACTION", "RESPOND", "CREATE_TASK", "UPDATE_CRM", "RESPOND_AND_UPDATE", "ESCALATE", "REQUEST_MORE_INFORMATION"] },
    confidence: { type: "number", minimum: 0, maximum: 1 },
    risk: { type: "string", enum: ["LOW", "MEDIUM", "HIGH", "CRITICAL"] },
    reasoningSummary: { type: "string" },
    requiresResponse: { type: "boolean" },
    requiresApproval: { type: "boolean" },
    proposedToolActions: { type: "array", items: { type: "object", additionalProperties: false, required: ["tool", "operation", "arguments"], properties: { tool: { type: "string" }, operation: { type: "string" }, arguments: { type: "object", additionalProperties: true } } } },
  },
};
...
const responseJsonSchema = {
  type: "object",
  additionalProperties: false,
  required: ["subject", "bodyText", "bodyHtml", "confidence", "citations"],
  properties: {
...
export async function triageEmail(input: { email: string; businessName: string }): Promise<TriageResult> {
...
export async function decideEmail(input: { businessName: string; thread: string; triage: TriageResult; crm: unknown; rules: unknown }): Promise<DecisionResult> {
...
export async function draftEmail(input: { businessName: string; thread: string; triage: TriageResult; decision: DecisionResult; crm: unknown }): Promise<ResponseDraft> {
  const raw = await structured<unknown>("email_response", responseJsonSchema, `You are the RESPONSE pass for ${input.businessName}. Write only from verified thread and CRM context. Do not invent facts, prices, dates, commitments, legal conclusions, refunds, payment changes, or actions not present in context. Use concise natural paragraphs. Do not use em dashes.\n\nTRIAGE:\n${JSON.stringify(input.triage)}\n\nDECISION:\n${JSON.stringify(input.decision)}\n\nCRM:\n${JSON.stringify(input.crm)}\n\nTHREAD:\n${input.thread}`);
