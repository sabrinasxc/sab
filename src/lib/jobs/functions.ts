...
import { renewExpiringGmailWatches } from "@/integrations/gmail/watch";
...
export const renewGmailWatchesJob = inngest.createFunction({ id: "renew-gmail-watches", retries: 2 }, { cron: "17 6 * * *" }, async ({ step }) => step.run("renew-watches", renewExpiringGmailWatches));
export const functions = [ingestGmailHistoryJob, processIncomingEmailJob, followupJob, renewGmailWatchesJob];
